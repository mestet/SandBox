 ...
 * You've been given the task of fixing them, but after you wrote up new shifts you realized that some bunny workers
 * had been transferred to a different area and aren't available for their assigned shifts.
 * Manually sorting through each shift list to compare
 * against work area lists will take forever -- remember, Commander Lambda loves efficiency!
 *
 ...
 */
public class TimeShiftChallenge {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
                solution(new int[]{13, 5, 6, 2, 5},
                        new int[]{5, 2, 5, 13}));

        System.out.println(
                solution(new int[]{14, 27, 1, 4, 2, 50, 3, 1},
    ...
    public static int solution(int[] x, int[] y) {
        ...
        if (x.length > y.length) {
            result = findMissing(x, y);
        } else {
            result = findMissing(y, x);
        }
        return result;
    }

    private static int findMissing(int[] longArr, int[] shortArr) {
        int result = 0;
        for (int i = 0; i < shortArr.length; i++) {
            result ^= longArr[i];
            result ^= shortArr[i];
        }
        result ^= longArr[longArr.length - 1];
        return result;
    }
}
